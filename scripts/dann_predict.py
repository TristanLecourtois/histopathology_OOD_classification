...
state_dict = load('best_dann.pth')
num_domains = state_dict['domain_clf.2.weight'].shape[0]
...
with torch.no_grad():
    for fname in test_files:
        ...
        label_logits, _ = model(feats, alpha=0.0)
        probs = F.softmax(label_logits, dim=1)[:, 1].cpu()
...
out_path = os.path.join(OUTPUT_DIR, f'submission_{args.model}_dann.csv')
